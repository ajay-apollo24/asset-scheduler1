-- Unified Campaign Migration Script for pgAdmin (FIXED)
...
-- Step 5: Update the status check constraint to include 'pending' and 'approved' statuses
ALTER TABLE campaigns DROP CONSTRAINT IF EXISTS campaigns_status_check;
ALTER TABLE campaigns ADD CONSTRAINT campaigns_status_check 
CHECK (status IN ('draft', 'pending', 'approved', 'active', 'paused', 'completed', 'rejected'));

-- Step 6: Create a view to maintain backward compatibility with existing bookings
...
-- Step 7: Create a function to migrate existing bookings to campaigns
...
BEGIN
    ...
    LOOP
        -- Map booking status to campaign status
        CASE booking_record.status
            WHEN 'pending' THEN mapped_status := 'pending';
            WHEN 'approved' THEN mapped_status := 'approved';
            WHEN 'rejected' THEN mapped_status := 'rejected';
            ELSE mapped_status := 'draft'; -- Default fallback
        END CASE;
        ...
        INSERT INTO campaigns (
            ...
            COALESCE(booking_record.auction_status, 'none'),
            'manual',
...
-- Step 8: Create indexes for the new unified system
...
-- Step 9: Create a unified allocation function
...
-- Step 10: Create a unified bidding function
...
-- Step 12: Create a trigger to maintain data consistency
...
-- Step 13: Create a function to get unified analytics
...
-- Step 14: Migrate existing bookings to campaigns
...
-- Step 16: Display migration summary
DO $$
DECLARE
